...
private final class NativeOwner: @unchecked Sendable {
  ...
  static func open(_ pack: IchiranInstalledPack) throws -> NativeOwner {
    guard ichiran_kernel_abi_version() == 3 else {
      throw IchiranAnalyzerError(
        code: .internal,
        message: "Ichiran kernel ABI mismatch: Swift requires version 3"
      )
    }
    let hot: Data
    do {
  ...
  func legacy(units: [UInt16], options: Data) throws -> Data {
    ...
    defer { ichiran_legacy_operation_free(operation) }

    var suppliedEntry = UInt32.max
    var supplied = Data()
    for _ in 0..<4_096 {
      let step = supplied.withUnsafeBytes { bytes in
      ...
      let data = try NativeResult.consume(step: step, fallback: .internal)
      if step.state == 1 { return data }
      guard step.state == 2 else {
        throw IchiranAnalyzerError(
          code: .internal, message: "Rust returned an invalid qualification step state")
      }
    ...
    throw IchiranAnalyzerError(
      code: .internal, message: "Qualification operation exceeded 4096 detail reads")
  }
