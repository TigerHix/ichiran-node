...
| 3 | `root-payload.ts:309` | Root seq, form counts, primary-no-kanji, archived and prefer-kana flags. | `CanonicalEntry` plus final chronological mutations. |
| 4 | `root-payload.ts:344` | Active root POS values. | Ordered sense properties after archived-sense filtering. |
| 5 | `root-payload.ts:356` | Direct kanji/kana forms; `row_number` by `ctid DESC` becomes lookup precedence. | Canonical forms with explicit creation-event order; baseline CTID conflict is the M2 blocker. |
| 6 | `root-payload.ts:398` | Restricted reading/written pairs. | JMdict `re_restr` facts. |
| 7 | `root-payload.ts:415` | Unordered legacy lookup followed by `unshift`, used to prove the direct order. | Oracle-only parity check. |
| 8 | `details.ts:448` | Root entry sequence list. | Canonical roots ordered by seq. |
| 9 | `details.ts:454` | Direct forms and form metadata. | Canonical forms. |
| 10 | `details.ts:484` | Senses and ordinals. | Canonical senses. |
| 11 | `details.ts:491` | Glosses and ordinals. | Canonical glosses. |
| 12 | `details.ts:502` | Sense properties ordered by tag, ord and physical id. | `(tag, ordinal, propertySourceOrder)`; 75 qualified equal-tag/ord groups require the last field. |
| 13 | `morphology-compiler.ts:158` | Conjugating root/POS pairs. | Forward emission roots and ordered POS facts. |
| 14 | `morphology-compiler.ts:185` | Root form sets. | Canonical form sets. |
| 15 | `morphology-compiler.ts:197` | Two manual patch families (`では` and expression cases). | Named compatibility declarations with upstream code provenance. |
...
| 18 | `analyzer-support.ts:986` | Generated/lexical collision paths ordered by physical conjugation/property ids. | Canonical lineage keys, including `via`. |
| 19 | `analyzer-support.ts:1012` | Collision-entry flags and POS. | Canonical lexical entries. |
| 20 | `analyzer-support.ts:1060` | First-stage collision lineage. | Explicit first emission/member lineage. |
| 21 | `analyzer-support.ts:1235` | Suffix conjugation/property hydration. | Suffix declarations joined to semantic member/property ordinals. |
| 22 | `analyzer-support.ts:1325` | Direct forms required by split/hint annotations. | Canonical forms. |
| 23 | `analyzer-support.ts:1415` | Generated split locators. | Forward emission locators. |
| 24 | `analyzer-generated.ts:123` | Primary/secondary paths, rule aliases, manual patches, physical target reuse, member/property/via ordinals and overlay facts. | One forward `ConjugationEmission` stream folded into a separate physical-target model. |
| 25 | `analyzer-generated.ts:314` | Generated lookup classes ranked through `max(form.ctid)`. | Complete semantic class key and explicit creation-event precedence; remaining CTID baseline conflicts require a reviewed choice. |
