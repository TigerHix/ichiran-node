...
with the unchanged expression entry 2268020. The frozen detailed presenter adds
`Past (~ta) via Passive` while the selected entry, segmentation, and score stay
unchanged. The clean product witness `一本とられる` selects entry 1859020 with the
exact Passive inflection. Both observations are accepted only for the exact September 2
source-lock digest.
