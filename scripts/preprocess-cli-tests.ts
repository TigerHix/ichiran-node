#!/usr/bin/env tsx
// Script to preprocess CLI comparison tests by running Docker once and caching outputs
...
function main() {
  ...
  const testCasesPath = join(__dirname, '../tests/data/cli.json');
  ...
  const outputPath = join(__dirname, '../tests/data/cli-lisp-outputs.json');
