...
import { analyzeSentence } from '../src/grammarMatcher/runtime.js';
...
async function main() {
  ...
  hr('ANALYZE PROFILE (analyzeSentence)');
  const a0 = performance.now();
  const result = await analyzeSentence(SAMPLE, grammarCatalog, { maxMatches: 100 }, 5);
