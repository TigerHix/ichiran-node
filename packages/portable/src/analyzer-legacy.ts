  ...
  const render = async (
    ...
    for (const row of mergeViaRows(stageRows(items))) {
      ...
      if (row.via) {
        ...
        if (via.length > 0) {
          nodes.push({ prop, via, readok: via[0]!.readok });
        }
        continue;
      }
      ...
      const labelRoute = root && testWord(root.form, 'kana') ? 'kana' : 'kanji';
      const hintedReading = root
        ? hydrate.context.hint?.(root.seq, 'kana', root.reading, root.reading)
          ?? hydrate.context.hint?.(root.seq, labelRoute, root.form, root.reading)
          ?? root.reading
        : '';
      const gloss = entry && root && member.entryIndex !== null
        ? senses(
            entry,
            member.entryIndex,
            hydrate.context,
      ...
      nodes.push({
        prop,
        reading: root
          ? readingLabel(labelRoute, root.form, hintedReading, null)
          : '',
        gloss,
        readok: root !== null
      });
...
async function detailedWord(
  ...
  if (word.seq === null) return output;
