...
import { withConnectionOverride } from '../../core/src/conn.js';
import { resetCalcScoreCache } from '../../core/src/dict/cache.js';
import {
...
function usage(message?: string): never {
  ...
  console.error(`usage: bun packages/portable/tools/oracle-parity.ts \\
...
async function openReference(database: string): Promise<CoreReference> {
  ...
  const core = await import('../../core/src/index.ts') as unknown as CoreReference['core'];
  const pool = postgres({
    host: spec.host,
    port: spec.port ?? 5432,
