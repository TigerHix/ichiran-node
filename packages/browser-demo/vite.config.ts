...
const OPAQUE_ANALYZER_ASSET = /^\/analyzer\/(hot|lexicon|gloss\.[A-Za-z0-9-]+)\.bin\.gz$/;

function installOpaqueAnalyzerMiddleware(
  ...
  middlewares.use((request, response, next) => {
    ...
    const match = OPAQUE_ANALYZER_ASSET.exec(pathname);
    if (!match) return next();
    const filename = `${match[1]}.bin.gz`;
    const path = resolve(analyzerDirectory, filename);
...
function serveOpaqueAnalyzerAssets(): Plugin {
  return {
    ...
    configureServer(server) {
      installOpaqueAnalyzerMiddleware(
        server.middlewares,
        resolve(server.config.publicDir, 'analyzer')
      );
    },
    configurePreviewServer(server) {
      installOpaqueAnalyzerMiddleware(
        server.middlewares,
        resolve(server.config.root, server.config.build.outDir, 'analyzer')
      );
    }
  };
}
