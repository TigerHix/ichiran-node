...
use crate::analyzer_lexicon::{
    AnalysisInflection, AnalysisRoot, AnalyzerLexicon, CandidateKind, ConjugationSelection,
    MaterializedCandidate, SemanticMember,
};
use crate::analyzer_model::{
    Conjugation, ConjugationProperty, EntryScoreFacts, ScoreCandidate, SequenceFacts,
    WordScoreFacts,
};
use crate::error::{ErrorCode, KernelError, Result};
use crate::morphology::Route;
use crate::roots::RootPayload;
use crate::support::{AnalyzerSupport, SupportConjugation, SupportConjugations, SupportSuffixForm};

type ScoreSplitResolver<'a> =
    dyn Fn(u32, Route, &[u16]) -> Result<Option<crate::analyzer_model::ScoreSplit>> + 'a;

pub struct SuffixMaterializer<'a, 'lexicon> {
    roots: &'a RootPayload,
...
impl<'a, 'lexicon> SuffixMaterializer<'a, 'lexicon> {
    pub fn new(
        roots: &'a RootPayload,
        support: &'a AnalyzerSupport,
        lexicon: &'a mut AnalyzerLexicon<'lexicon>,
    ...
    pub fn select_conjugations(
        ...
        let scoring_members: Vec<_> = semantic_members
            ...
            .collect();
        let lineage_facts =
            member_sequence_facts(self.roots, &candidate.semantic_members, &scoring_members)?;
    ...
    pub fn suffix_component(
        &mut self,
        form: &SupportSuffixForm,
    ) -> Result<Option<MaterializedCandidate>> {
        let text = utf16(&form.text);
        let values = self.lexicon.lexical(&text)?;
        ...
        let inherited_common = candidate_word.inherited_common;
        let inherited_ord = candidate_word.inherited_ord;
        ...
        let score_facts = ScoreCandidate::Word(WordScoreFacts {
            ...
            route: Route::Kana,
            seq: Some(i64::from(if selection == ConjugationSelection::Root {
                root_seq
            } else {
                form.seq
            })),
            ord: i32::from(form.ord),
            common: (selection == ConjugationSelection::Root)
                .then(|| form.common.map(i32::from))
                .flatten(),
        ...
        result.physical_key = crate::analyzer_lexicon::PhysicalKey::Sequence(physical_seq);
...
fn member_sequence_facts(
    ...
    selected_members
        .iter()
        .map(|member| {
            let mut facts = sequence_facts(roots, member.entry_index)?;
            let root_seq = member
                .root
                .as_ref()
                .map(|root| root.seq)
