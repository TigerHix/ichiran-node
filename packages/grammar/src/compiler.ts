...
// Helper function to skip Japanese comma tokens
function skipCommas(tokens: Token[], index: number): number {
...
function compileSequence(node: SequenceNode): Matcher {
  ...
  return async (tokens, index) => {
    ...
    async function dfs(depth: number, currentIndex: number, captures: MatchOutcome['captures'], preference: number): Promise<void> {
      ...
      // Skip commas before matching (except at the very start)
      const indexAfterCommas = depth > 0 ? skipCommas(tokens, currentIndex) : currentIndex;

      const matcher = matchers[depth];
      const results = await matcher(tokens, indexAfterCommas);
