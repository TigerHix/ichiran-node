...
export default linguisticRule('verbて-b', (r) => {
  ...
  // All te-forms have dep=advcl when connecting to another verb/phrase
  r.either(
    // 連用形-一般 (standard conjunctive, e.g., 食べて, 勉強して)
    (b1) => {
      const verb = b1.verb({ inflectionForm: '連用形-一般', dep: 'advcl' }, 'verb');
    ...
    (b2) => {
      const verb = b2.verb({ inflectionForm: '連用形-イ音便', dep: 'advcl' }, 'verb');
    ...
    (b3) => {
      const verb = b3.verb({ inflectionForm: '連用形-撥音便', dep: 'advcl' }, 'verb');
    ...
    (b4) => {
      const verb = b4.verb({ inflectionForm: '連用形-促音便', dep: 'advcl' }, 'verb');
    ...
    (b5) => {
      const verb = b5.verb({ inflectionForm: '連用形-ウ音便', dep: 'advcl' }, 'verb');
