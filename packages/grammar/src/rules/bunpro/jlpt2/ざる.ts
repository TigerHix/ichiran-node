...
export default linguisticRule('ざる', (r) => {
  r.excludedBy(['ざるを得ない']);
  r.either(
    // Pattern 1: ざる as auxiliary (AUX) with dep=aux
    // Most common pattern: verb stem + auxiliary ざる
    (b) => {
      const zaru = b.aux({
        text: 'ざる',
        dep: 'aux',
    ...
    // Pattern 2: ざる as particle (PART) with dep=mark
    ...
    // Pattern 3: ざる with lemma indicating classical negative form
    ...
    // Pattern 4: ざる as VERB (GiNZA sometimes mis-tags classical auxiliaries)
    (b) => {
      const zaru = b.verb({
        text: 'ざる',
      }, 'zaru');
    ...
    // Pattern 5: ざる following a verb stem (連用形)
    // Ensures it's acting as auxiliary, not independent word
    (b) => {
      const stem = b.verb({
