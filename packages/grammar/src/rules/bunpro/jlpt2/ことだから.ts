...
export default linguisticRule('ことだから', (r) => {
  r.either(
    ...
    // Pattern 5: Verb + こと + だから (looser dependencies, more distance allowed)
    (b5) => {
    ...
    // Pattern 7: ...とのこと + だから (quoted clause + こと)
    // Example: お相手が「感激した」とのことだから、結果的にはあれで良かったと思います。
    (b7) => {
      const to = b7.particle('と', 'to');
    ...
    // Pattern 8: Any token + の + こと + だから (very loose, for quoted patterns)
    (b8) => {
      const prev = b8.tok({}, 'prev');
      const no = b8.particle('の', 'no');
      const koto = b8.noun({ text: 'こと', lemma: 'こと' }, 'koto');
      const da = b8.aux({ text: 'だ', lemma: 'だ' }, 'da');
      const kara = b8.particle('から', 'kara');

      b8.inOrder(prev, no, 1);
      b8.inOrder(no, koto, 1);
      b8.inOrder(koto, da, 1);
      b8.inOrder(da, kara, 1);

      b8.captureSpan('ことだから', prev, kara);
    }
  );
});
