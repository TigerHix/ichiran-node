import { type ConjInfoJson, type WordInfoGlossJson, WordInfo } from "@ichiran/core/src/dict/presentation.js"
import { type TransformedRomanizeStarResult, type TransformedRomanizeStarResultTokenTuple } from "@ichiran/core/src/presentation/transformers.js"
import { type RomanizeStarResult, type RomanizeStarResultTokenTuple } from "@ichiran/core/src/romanize.js"
...
function convertIchiranConjugationToGrammarToken(conj: ConjInfoJson): GrammarInfo {
    ...
    return {
        isConjugation: true,
        conjugationTypes: conj.prop?.map((prop: any) => prop.type),
        neg: conj.prop?.some((prop: any) => prop.neg) || undefined,
        fml: conj.prop?.some((prop: any) => prop.fml) || undefined,
        ...
        meanings: conj.gloss?.map((gloss: any) => ({
            text: gloss.gloss,
            partOfSpeech: parsePartOfSpeech(gloss.pos),
            info: gloss.info || '',
        })) || [],
        conjugations: conj.via?.map((via: any) => ({
...
function extractConjugations(wordInfo: WordInfoGlossJson): GrammarInfo[] | undefined {
    ...
    return wordInfo.conj.map((conj: any) => ({
...
function extractAlternatives(wordInfo: WordInfoGlossJson): GrammarInfo[] | undefined {
    ...
    return wordInfo.alternative.map((alternative: any) => ({
...
function extractComponents(wordInfo: WordInfoGlossJson): GrammarInfo[] | undefined {
    ...
    return wordInfo.components.map((component: any) => ({
