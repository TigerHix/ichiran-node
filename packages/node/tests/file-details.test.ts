...
async function fixture(installed = Buffer.from('0123456789')): Promise<{
  ...
  await writeFile(join(release, 'details.bin.gz'), downloaded);
  return {
    release,
    temporary,
    installed,
...
describe('file-backed Node details', () => {
  test('verifies once, serves exact positional ranges, and removes owned bytes on dispose', async () => {
    const value = await fixture();
    const source = await openVerifiedDetailSource(value.release, value.asset, value.temporary);
  ...
  test('rejects a compressed checksum mismatch and removes the partial spool', async () => {
    const value = await fixture();
    await expect(openVerifiedDetailSource(value.release, {
      ...value.asset,
  ...
  test('uses a verified identity asset in place and never deletes release data', async () => {
    const value = await fixture();
    const path = join(value.release, 'details.bin');
    ...
    const source = await openVerifiedDetailSource(value.release, asset, value.temporary);
