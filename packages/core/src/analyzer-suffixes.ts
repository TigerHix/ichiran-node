...
const CONDITIONAL_ABBREVIATIONS: Readonly<Record<string, string>> = Object.freeze({
  ':teba': 'てば', ':reba': 'れば', ':keba': 'けば', ':geba': 'げば',
  ':neba': 'ねば', ':beba': 'べば', ':meba': 'めば', ':seba': 'せば'
});

export interface AnalyzerSuffixSource {
  readonly roots: RootPayloadReader;
  readonly support: AnalyzerSupportReader;
  readonly annotations: PortableAnalyzerAnnotations;
}

/** Recursive split, conjugation, and suffix candidate resolution. */
export class AnalyzerSuffixResolver {
  readonly #roots: RootPayloadReader;
  readonly #support: AnalyzerSupportReader;
  readonly #annotations: PortableAnalyzerAnnotations;
  ...
  constructor(source: AnalyzerSuffixSource, lexicon: AnalyzerLexicon) {
  ...
  uniqueSuffix(suffixClass: string, matches: readonly MaterializedCandidate[]): boolean {
  ...
  withSuruBreak(candidate: AnalyzerScoreCandidate): AnalyzerScoreCandidate {
    ...
    for (let index = matches.length - 1; index >= 0; index--) {
      ...
      for (const value of match.values) {
        ...
        return { ...candidate, suruBreak: { suffixText: match.text, candidate: suffix.scoreFacts } };
      }
    }
    return candidate;
  }
  ...
  applySuffix(
    ...
    switch (keyword) {
      ...
      case ':rashii': {
        ...
        primary = dedupeCandidates([...first, ...second]);
        modifier = scoreModifier(3); break;
      }
      case ':desu':
        if (root.endsWith('ない') || root.endsWith('なかった')) {
          primary = this.#withProperty(root, negative, false, next);
        }
    ...
    return primary.map(value => this.#compound(
      value, suffixCandidate, suffix, surface, stem, connector, modifier, patch,
      keyword === ':suru'
    ));
  }
  ...
  #splitPart(part: AnalyzerSupportSplitPart): MaterializedCandidate | null {
    ...
    const values = this.#lexicon.lexical(part.text).filter(value => value.route === part.route);
    ...
    const signature = (rows: readonly {
      ...
      value.from, value.via, value.pos, value.type, value.negative, value.formal
    ])))].sort());
    const wanted = signature(part.generated);
    return values.find(value => signature(value.semanticMembers.flatMap(member => {
      const property = member.inflection.at(-1);
      const from = member.root?.seq ?? member.publicSeq;
      if (!property || from === null) return [];
      return [{
        from, via: member.inflection.length > 1, pos: property.pos, type: property.type,
  ...
  #selectConjugations(
    ...
    if (candidate.kind !== 'simple' || candidate.scoreFacts.kind === 'compound') return candidate;
    ...
    const conjugationFor = (member: CandidateSemanticMember): AnalyzerConjugation | null => {
      ...
      return {
        ...
        property: {
          pos: property.pos, type: property.type,
          negative: property.negative, formal: property.formal
        }
      };
    };
    ...
    const semanticMembers = candidate.semanticMembers.filter(member => selectedRows.has(rowKey(member)));
    const seenConjugations = new Set<string>();
    const conjugations = semanticMembers.flatMap(member => {
      const conjugation = conjugationFor(member);
      if (!conjugation) return [];
      const key = JSON.stringify([rowKey(member), conjugation.from, conjugation.via, conjugation.property]);
    ...
    const scoringMembers = semanticMembers.filter(member => secondaryOnly
      ...
      : member.inflection.length === 1);
    const lineageFacts = this.#lexicon.memberSequenceFacts(candidate.semanticMembers, scoringMembers);
    const targetFacts = wordFacts.self;
    return withKey({
      ...candidate,
      ...
      scoreFacts: {
        ...
        entry: wordFacts.entry === null ? null : {
          ...wordFacts.entry,
          nKanji: preserveDirectTarget
            ? wordFacts.entry.nKanji
            : selected.targetNKanji ?? wordFacts.entry.nKanji
        }
      },
      semanticMembers,
  ...
  #withPos(surface: string, wanted: readonly string[], depth: number): MaterializedCandidate[] {
    void depth;
    return this.#lexicon.lexical(surface).filter(value => {
      const facts = value.scoreFacts;
  ...
  #suffixComponent(form: AnalyzerSupportSuffixForm): MaterializedCandidate | null {
    const values = this.#lexicon.lexical(form.text);
    ...
    const selection: MaterializedCandidate['conjugationSelection'] = form.conjugations === ':root'
      ? 'root'
    ...
    const rootSeq = selectedMember?.root?.seq
      ...
      ?? candidate.publicSeq
      ?? form.seq;
    const rootEntryValue = selectedMember?.entryIndex ?? this.#roots.findEntryIndex(rootSeq);
    ...
    const scoreFacts: AnalyzerWordScoreFacts = {
      kind: 'word', text: form.text, trueText: form.text, trueTextFollowsText: true,
      route: 'kana', seq: form.conjugations === ':root' ? rootSeq : form.seq,
      ord: form.ord, common: form.conjugations === ':root' ? form.common : null,
    ...
    return withKey({
      ...candidate,
      kind: 'simple', text: form.text, trueText: form.text, route: 'kana', reading: form.text,
  ...
  #compound(
    ...
    const primaryComponents = primary.kind === 'compound'
      ...
      : [{
          text: primary.text, trueText: primary.trueText === primary.text ? null : primary.trueText,
          route: primary.route, reading: primary.reading, entryIndex: primary.entryIndex,
          root: primary.root, inflection: primary.inflection, primary: true,
          publicSeq: primary.publicSeq, physicalKey: primary.physicalKey,
          physicalGroup: primary.physicalGroup, suffixClass: primary.suffixClass,
          definitionSeq: primary.definitionSeq, semanticMembers: primary.semanticMembers,
          identityRoots: primary.identityRoots,
          conjugationSelection: primary.conjugationSelection
        }];
    const components = [...primaryComponents, {
      text: suffix.text, trueText: suffix.trueText === suffix.text ? null : suffix.trueText,
      route: suffix.route, reading: suffix.reading, entryIndex: suffix.entryIndex,
      root: suffix.root, inflection: suffix.inflection,
      primary: suffix.physicalKey === primary.physicalKey, publicSeq: suffix.publicSeq,
      physicalKey: suffix.physicalKey, physicalGroup: suffix.physicalGroup,
      suffixClass: suffix.suffixClass, definitionSeq: suffix.definitionSeq,
      semanticMembers: suffix.semanticMembers, identityRoots: suffix.identityRoots,
      conjugationSelection: suffix.conjugationSelection
    }];
    const previousCompound = primary.scoreFacts.kind === 'compound' ? primary.scoreFacts : null;
    ...
    const scoreFacts: AnalyzerScoreCandidate = {
      kind: 'compound', text: surface, base, modifier: combinedModifier,
    ...
    return withKey({
      ...
      physicalKey: primary.physicalKey, physicalGroup: primary.physicalGroup,
      lookupLocators: [], memberOrd: primary.memberOrd,
      ...
      definitionSeq: primary.definitionSeq, conjugationSelection: 'default',
      semanticMembers: primary.semanticMembers, identityRoots: primary.identityRoots
    });
  }
  ...
  #abbreviations(
    ...
    return primary.map(value => {
      ...
      if (value.kind === 'compound') {
        return withKey({
          ...value, text: surface, trueText: surface, reading,
          scoreFacts: { ...value.scoreFacts, text: surface }
        });
      }
      const facts = value.scoreFacts as AnalyzerWordScoreFacts;
      return withKey({
        ...value, kind: 'proxy', text: root + suffix, reading,
        scoreFacts: {
          ...facts, text: root + suffix,
          trueText: facts.trueText, trueTextFollowsText: false
        }
      });
    });
  }
