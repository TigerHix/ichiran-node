// ichiran/dict/splitDefinitions - Word splitting definitions
// Extracted from split.ts - contains split/hint definitions and maps
// Query functions moved to splitQueries.ts to break circular dependencies

import { trueText } from './utils.js';
import { trueKana, trueKanji } from './readings.js';
import { getKana } from './readings.js';
import { getText, getWordType } from './utils.js';
import { wordConjData } from './conjugation.js';
import { findWordSeq, findWordConjOf } from './suffixHelpers.js';
...
import type { CompoundText, Segment, Reading } from '../types.js';
import { isCompoundText, isSimpleWord, isWordWithText } from '../types.js';
// Import calcScore for split scoring
import { calcScore } from './scoring.js';
// Import maps and types from splitMaps.ts (we populate them here)
import type { SplitAttrs, AsyncSplitFunction, HintFunction } from './splitMaps.js';
import { splitMap, hintMap, segsplitMap } from './splitMaps.js';

// =============================================================================
// SPLIT/HINT DEFINITION FUNCTIONS
// =============================================================================
    ...
    for (const partDef of parts) {
      ...
      if (partDef.type === 'part') {
        ...
        if (typeof partDef.seqs === 'number') {
          ...
          if (found.length > 0 && isSimpleWord(found[0])) {
            pseqs = [found[0].seq];
          } else {
            pseqs = [];
          }
...
// Query functions (getSplit_, getSplitImpl) moved to splitQueries.ts
...
function defTooriSplit(seq: number, seqA: number, seqB: number = 1432930, score: number = 50): void {
  defSimpleSplit({
    ...
    parts: [
      {
        type: 'test',
        condition: (_len, _txt, reading) => getWordType(reading) === 'kanji'
      },
...
defSimpleSplit({
  ...
  score: 10,
  parts: [
    { type: 'test', condition: (_len, _txt, r) => getWordType(r) === 'kana' },
...
defSimpleSplit({
  ...
  score: 50,
  parts: [
    { type: 'test', condition: (_len, _txt, r) => getWordType(r) === 'kanji' },
...
// segsplitMap is imported from splitMaps.ts
// In Lisp, they use (let ((*split-map* *segsplit-map*)) ...)
// We define these splits manually with segsplitMap
...
export async function getSegsplit(segment: Segment): Promise<Segment | null> {
  ...
  if (!word || !isWordWithText(word) || isCompoundText(word) || !isSimpleWord(word)) {
    return null;
  }
  ...
  const getSeqs = (word: any): number[] => {
    if (isSimpleWord(word)) {
      ...
      return Array.isArray(word.seq) ? word.seq : [];
    }
    return [];
  };
  ...
  if (newSeg.info) {
    ...
    // Create minimal CalcScoreInfo with just conjugation data
    newSeg.info = {
      posi: [],
      seqSet: [],
      conj: conjData,
...
async function getSegsplit_(reading: Reading, conjOf: number[] = []): Promise<[any[], SplitAttrs] | null> {
  const readingSeq = isSimpleWord(reading) ? reading.seq : null;
...
// hintMap is imported from splitMaps.ts
...
// Query function (getHintImpl) moved to splitQueries.ts
