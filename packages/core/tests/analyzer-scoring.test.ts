...
} from '../../reference-postgres/src/dict/scoring.js';
import {
...
describe('portable analyzer scoring', () => {
  test('locks representative root, weak conjugation, particle, and skip scores', () => {
    ...
    // Upstream Jan 2026 removed the deleted てもいい row from this rule and
    // classified the newly added synthetic まう entry as suffix-only instead.
    expect(scoreAnalyzerCandidate(word({ seq: 2_458_040 })).score).toBeGreaterThan(0);
    expect(scoreAnalyzerCandidate(word({ seq: 2_827_357 })).score).toBe(0);
  });
