...
import { getPenalties as currentPenalties } from '../../reference-postgres/src/grammar/penalties.js';
import { applySegfilters as currentSegfilters } from '../../reference-postgres/src/grammar/segfilters.js';
import { getSynergies as currentSynergies } from '../../reference-postgres/src/grammar/synergies.js';
import type { CalcScoreInfo, Segment, SegmentList } from '../../reference-postgres/src/types.js';
import {
...
describe('portable analyzer-internal path rules', () => {
  ...
  test('differentially matches filtering, penalty priority, and non-adjacent behavior', async () => {
    ...
    const mononi = applyAnalyzerSegfilters(
      portableGroup(1, 0, 1, [{ id: 120, text: 'も', seqSet: [2028940] }, neutral]),
      portableGroup(2, 1, 3, [
        { id: 121, text: 'のに', seqSet: [1009980] },
        { ...neutral, id: 122 }
      ])
    );
    expect(mononi.map(([left, right]) => ({
      left: left === null ? [] : portableIds(left),
      right: portableIds(right)
    }))).toEqual([
      { left: [120, neutral.id], right: [122] },
      { left: [neutral.id], right: [121] }
    ]);

    // This is an intentional ea958336 analyzer delta beyond the frozen d583
    // PostgreSQL reference: も cannot directly complement のに, but other
    // left and right alternatives survive the pair-level filter.
    const frozen = currentSegfilters(
      coreGroup(0, 1, [{ id: 120, text: 'も', seqSet: [2028940] }, neutral]),
      coreGroup(1, 3, [
        { id: 121, text: 'のに', seqSet: [1009980] },
        { ...neutral, id: 122 }
      ])
    );
    expect(coreIds(frozen[0]![0]!)).toEqual([120, neutral.id]);
    expect(coreIds(frozen[0]![1])).toEqual([121, 122]);
