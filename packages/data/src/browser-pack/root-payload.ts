...
export interface RootPayloadFormSource {
  ...
  readonly seq: number;
  /**
   * Dense semantic lookup order. The migration oracle derives the qualified
   * value from legacy heap order; the source compiler supplies its explicit
   * deterministic replacement. No physical tuple or surrogate row id crosses
   * this builder boundary.
   */
