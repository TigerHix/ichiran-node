...
export const CHAR_CLASS_HASH = new Map();
...
export function longVowelModifierP(modifier, prevChar) {
    const vowelMap = {
        "+a": "A", "+i": "I", "+u": "U", "+e": "E", "+o": "O"
    };
...
export const HANDAKUTEN_HASH = new Map([
    ["ha", "pa"], ["hi", "pi"], ["fu", "pu"], ["he", "pe"], ["ho", "po"]
]);
// Line 73-79: hash-from-list *undakuten-hash*
export const UNDAKUTEN_HASH = new Map([
...
function dakutenJoin(dakutenHash, char) {
    const result = [];
    for (const [cc, ccd] of dakutenHash.entries()) {
        let kc = KANA_CHARACTERS[cc];
        let kcd = KANA_CHARACTERS[ccd];
        if (!kc || !kcd)
            continue;
...
export const ABNORMAL_CHARS = "０１２３４５６７８９ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ＃＄％＆（）＊＋／〈＝〉？＠［］＾＿'｛｜｝～" +
    HALF_WIDTH_KANA;
// Line 114-116: defparameter *normal-chars*
export const NORMAL_CHARS = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ#$%&()*+/<=>?@[]^_`{|}~" +
    FULL_WIDTH_KANA;
...
export const CHAR_CLASS_REGEX_MAPPING = {
    "katakana": /[ァ-ヺヽヾー]/,
    "katakana-uniq": /[ァ-ヺヽヾ]/,
    "hiragana": /[ぁ-ゔゝゞー]/,
    "kanji": /[々ヶ〆一-龯]/,
...
export function collectCharClass(word, charClass) {
    const regex = CHAR_CLASS_REGEX_MAPPING[charClass];
    if (!regex)
        return [];
    const matches = word.match(new RegExp(regex.source, 'g'));
...
export function sequentialKanjiPositions(word, offset = 0) {
    const positions = [];
...
export function kanjiMask(word) {
    return word.replace(/[々ヶ〆一-龯]+/g, '%');
}
// Line 190-198: defun kanji-regex
export function kanjiRegex(word) {
    const mask = kanjiMask(word);
    const pattern = mask.split('').map(char => char === '%' ? '.+' : char.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('');
    return new RegExp(`^${pattern}$`);
}
// Line 200-201: defun kanji-match
export function kanjiMatch(word, reading) {
    return kanjiRegex(word).test(reading);
}
// Line 203-208: defun kanji-cross-match
export function kanjiCrossMatch(word, reading, newWord) {
    let m = 0;
...
export function simplifyNgrams(str, map) {
    let result = str;
...
function toNormalChar(char, context) {
...
export function normalize(str, context, skipPunctuation = false) {
...
export function splitByRegex(regex, str) {
...
export function basicSplit(str) {
...
export function moraLength(str) {
    return str.split('').filter(char => !"っッぁァぃィぅゥぇェぉォゃャゅュょョー".includes(char)).length;
}
// Line 251-260: defun as-hiragana
export function asHiragana(str) {
    return str.split('').map(char => {
        const normalized = toNormalChar(char) ?? char;
        const charClass = CHAR_CLASS_HASH.get(normalized);
        if (charClass) {
            const chars = ALL_CHARACTERS[charClass];
...
export function asKatakana(str) {
    return str.split('').map(char => {
        const normalized = toNormalChar(char) ?? char;
        const charClass = CHAR_CLASS_HASH.get(normalized);
        if (charClass) {
...
export function consecutiveCharGroups(charClass, str, start = 0, end = str.length) {
    const result = [];
...
export function kanjiPrefix(word) {
    const match = word.match(/^.*[々ヶ〆一-龯]/);
    return match ? match[0] : "";
}
// Line 286-296: defun unrendaku
export function unrendaku(txt, fresh = false) {
    ...
    if (!unvoiced)
        return result;
    const kanaChars = KANA_CHARACTERS[cc];
    const unvoicedChars = KANA_CHARACTERS[unvoiced];
    if (!kanaChars || !unvoicedChars)
        return result;
...
export function rendaku(txt, fresh = false, handakuten = false) {
    let result = fresh ? txt : txt;
    if (result.length === 0)
        return result;
    const firstChar = result[0];
    ...
    if (!voiced)
        return result;
    const kanaChars = KANA_CHARACTERS[cc];
    const voicedChars = KANA_CHARACTERS[voiced];
    if (!kanaChars || !voicedChars)
        return result;
    const pos = kanaChars.indexOf(firstChar);
    if (pos < 0)
        return result;
...
export function geminate(txt, fresh = false) {
...
export function destem(word, stem, charClass = 'kana') {
    if (stem === 0)
        return word;
    const regex = CHAR_CLASS_REGEX_MAPPING[charClass];
    const positions = [];
...
export function matchDiff(s1, s2) {
    const l1 = s1.length;
    const l2 = s2.length;
    if (l1 === 0 || l2 === 0)
        return null;
...
export function safeSubseq(sequence, start, end) {
    const len = sequence.length;
    if (start < 0 || start > len)
        return null;
    if (end !== undefined && (end < start || end > len))
        return null;
    return sequence.slice(start, end);
}
// Line 365-370: defun join
export function join(separator, list, key) {
