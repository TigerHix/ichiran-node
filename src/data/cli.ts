...
import { loadJMDict, getDbStats } from './load-jmdict.js';
...
import { loadKanjidic, getKanjiStats } from './load-kanjidic.js';
import { calculateBestReadings, calculateKanjiStatistics, calculateReadingStatistics } from './maintenance.js';
import { getConnection, validateDatabaseSafety } from '../conn.js';
import { downloadDataFile, ensureDataFile, dataFileExists, getDataPath } from './download.js';
...
program
  ...
  .action(async () => {
    try {
      const startTime = Date.now();
      const { addErrata } = await import('./errata.js');
      await addErrata();
      const elapsed = (Date.now() - startTime) / 1000;
