...
export class CalcScoreMemoizer {
  ...
  private serializeWord(word: any, depth: number = 0): any {
    ...
    if (isKanjiText(word)) base.bestKana = word.bestKana;
    if (isKanaText(word)) base.bestKanji = word.bestKanji;
    if (isSimpleWord(word)) base.nokanji = word.nokanji;
  ...
  getKey(reading: any, args: {
    ...
    if (isCompoundText(r)) {
      ...
      cacheKey.k = r.kana;
      cacheKey.conj = r.conjugations;
      if (isKanjiText(r)) cacheKey.bk = r.bestKana;
      if (isKanaText(r)) cacheKey.bkj = r.bestKanji;
    }
...
export class CalcScoreDataCache {
  ...
  private preferKanaBySeq = new Map<number, SenseProp[]>();
  ...
  async getOrFetch<T>(
    ...
    if (type === 'entry') {
      ...
      if (uniqueSeqs.length === 0) return [] as T;

      const cacheKey = JSON.stringify([...uniqueSeqs].sort((a, b) => a - b));
      ...
      this.setPreferKana(uniqueSeqs, result);
      return result as T;
    } else {
      // nonArchPosi
      const seqs = key as number[];
      ...
      const result = await fetcher(sql);
      this.setNonArchPosi(uniqueSeqs, result as any);
      return result;
    }
  }
  ...
  async prefetchSeqs(seqs: number[]) {
    ...
    const key = JSON.stringify([...uniqueSeqs].sort((a, b) => a - b));
    if (!this.fetchedPreferKana.has(key)) {
      const preferKana = await sql<SenseProp[]>`
        ...
        WHERE seq IN ${sql(uniqueSeqs)}
          ...
          AND text = 'uk'
      `;
      this.setPreferKana(uniqueSeqs, preferKana);
    }
    ...
    if (!this.fetchedNonArchPosi.has(key)) {
      const result = await sql<{ text: string }[]>`
        ...
        WHERE sp1.seq IN ${sql(uniqueSeqs)}
          ...
          AND sp2.id IS NULL
      `;
      this.setNonArchPosi(uniqueSeqs, result.map(r => r.text));
    }
  }
  ...
  getPreferKana(seqs: number[]): SenseProp[] | undefined {
    ...
    if (uniqueSeqs.length === 1) {
      const seq = uniqueSeqs[0];
      if (this.preferKanaBySeq.has(seq)) {
        return this.preferKanaBySeq.get(seq)!;
      }
    }
  ...
  setPreferKana(seqs: number[], data: SenseProp[]) {
    ...
    for (const seq of uniqueSeqs) {
      ...
      this.preferKanaBySeq.set(seq, bySeq.get(seq)!);
    }
